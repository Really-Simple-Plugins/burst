	...
	class burst_field {
		...
		public function process_save() {
			...
			if ( isset( $_POST['burst_nonce'] ) ) {
				...
				//Save the custom URL's for not Burst generated pages.
				$docs = BURST::$document->get_document_types();
				foreach ($docs as $document){
					if (isset($_POST["burst_".$document."_custom_page"])){
						$doc_id = intval($_POST["burst_".$document."_custom_page"]);
						update_option("burst_".$document."_custom_page", $doc_id );
						//if we have an actual privacy statement, custom, set it as privacy url for WP
						if ($document==='privacy-statement' && $doc_id > 0){
							BURST::$document->set_wp_privacy_policy($doc_id, 'privacy-statement');
						}
					}
					if (isset($_POST["burst_".$document."_custom_page_url"])){
						$url = esc_url_raw($_POST["burst_".$document."_custom_page_url"]);
						update_option("burst_".$document."_custom_page_url", $url );
					}
				}
		...
		function document(
			...
			$custom_page_id = get_option('burst_'.$args['fieldname'].'_custom_page');
			if ( ! $this->show_field( $args ) ) {
				return;
			}
			...
			?>
			<?php do_action( 'burst_before_label', $args ); ?>
			<label for="<?php echo $args['fieldname'] ?>"><?php printf(__("Select how you want to add your %s.",'burst'), $args['label']) ?><?php echo $this->get_help_tip_btn( $args ); ?></label>
			<?php do_action( 'burst_after_label', $args ); ?>

			<div class="burst-validate-radio burst-document-field" data-fieldname="<?php echo esc_html( $fieldname ) ?>">
				<input <?php if ( $args['disabled'] )
					echo "disabled" ?>
					<?php if ( $args['required'] ) {
						echo "required";
					...
					echo "checked" ?> class="burst-document-input">
				<label class="">
					<?php
						if ($fieldname === 'burst_cookie-statement'){
							echo apply_filters("burst_generate_document_label", __("Generated by Burst", "burst"), $args['fieldname']);
						} else {
							echo apply_filters("burst_generate_document_label", sprintf(__("Generate a comprehensive and legally validated %s with %spremium%s", "burst"), $args['label'],'<a href="https://burst.io/l/pricing/" target="_blank">', '</a>' ), $args['fieldname']);
						}
					 ?>
				</label>
				<div class="clear"></div>
				...
				<input
					...
					echo "checked" ?> class="burst-document-input">
				<label class="">
					<?php _e("Link to custom page", "burst"); ?>
				</label>
				...
				<?php if ( $args['fieldname'] !== 'cookie-statement' ){?>
					<input
						...
						<?php if ( $value == 'none' )
							echo "checked" ?> class="burst-document-input">
					<label class="">
						<?php printf( __("No document", "burst"), $args['label'] ); ?>
					</label>
					<div class="clear"></div>
				<?php } ?>
				<input type="text" class="burst-document-custom-url" value="<?php echo get_option($fieldname."_custom_page_url")?>" placeholder = "https://domain.com/your-policy" name="<?php echo esc_html( $fieldname."_custom_page_url" ) ?>">

				<select class="burst-document-custom-page" name="<?php echo esc_html( $fieldname."_custom_page" ) ?>">
					<option value=""><?php _e("None selected", "burst")?></option>
		...
		function upload(
			...
			<input type="file" type="submit" name="burst-upload-file"
			       value="<?php echo esc_html( $args['label'] ) ?>">
			<input <?php if ( $args['disabled'] )
				echo "disabled" ?> class="button" type="submit"
			                       name="<?php echo $args['action'] ?>"
		...
		function cookies(
			...
			<?php do_action( 'burst_before_label', $args ); ?>
			<div id="burst_action_success" class="burst-hidden">
				<?php echo burst_notice( __( 'Settings saved',
					'burst' ), 'success' ) ?>
			</div>

			<?php do_action( 'burst_after_label', $args ); ?>
			<?php
			$languages  = BURST::$cookie_admin->get_supported_languages();
			$count      = BURST::$cookie_admin->get_supported_languages( true );

			if ( $count > 1 ) {

				?>
				<select id="burst_language" class="burst_cookie_language_selector" data-type="cookie">
					<?php
				...
				?>
				<input type="hidden" id="burst_language" data-type="cookie"
				       value="<?php echo reset( $languages ) ?>">
				<?php
			}
			?>
			<label class="burst-switch">
				<input name="burst_show_deleted" size="40" type="checkbox"
				       value="1"/>
				<span class="burst-slider burst-round"></span>
			</label>
			<?php _e( "Show deleted cookies", "burst" ) ?>


			<div class="burst-list-container">
				<div class="burst-skeleton"></div>
			</div>
			<button type="button" class="button burst-edit-item"
			        name="burst_add_item" data-type='cookie' data-action="add"
		...
		private function get_language_descriptor( $language, $type = 'cookie' ) {
			$string = $type =='cookie' ? __( 'Cookies in %s', 'burst' ) : __( 'Services in %s', 'burst' );
			if ( isset( BURST::$config->language_codes[ $language ] ) ) {
				$string = sprintf( $string ,
					BURST::$config->language_codes[ $language ] );
			} else {
				$string = sprintf( $string,
					strtoupper( $language ) );
			}
		...
		function services(
			...
			if ( $count > 1 ) {
				?>
				<select id="burst_language" data-type="service">
					<?php
					foreach ( $languages as $language ) {
						?>
						<option
				...
				?>
				<input type="hidden" id="burst_language" data-type="service"
				       value="<?php echo reset( $languages ) ?>">
				<?php
			}
			?>
			...
			<button type="button" class="button burst-edit-item"
			        name="burst_add_item" data-type='service' data-action="add"
		...
		function processors(
			...
			$processing_agreements
				= BURST::$processing->processing_agreements();
			...
			<label><?php echo $args["label"] . " " . __( 'list',
						'burst' ) ?><?php echo $this->get_help_tip_btn( $args ); ?></label>
			<?php do_action( 'burst_after_label', $args ); ?>
			<?php
			if ( $values ) {
				foreach ( $values as $key => $value ) {
					...
					$create_processing_agreement_link = '<a href="'
					                                    . admin_url( "admin.php?page=burst-processing-$region" )
					                                    . '">';
					...
					$html = '<div class="multiple-field">
                        ...
                        <div>
                            <label>
                                <select name="burst_multiple['
					        . esc_html( $fieldname ) . '][' . esc_html( $key ) . '][processing_agreement]">
                                    <option value="0">'
					        . __( 'No agreement selected', 'burst' ) . '</option>
                                    <option value="-1" '
					        . $processing_agreement_outside_c . '>'
					        . __( 'A Processing Agreement outside Burst Privacy Suite',
							'burst' ) . '</option>';
					...
					$html .= '<input class="button" type="submit" name="burst-save" value="'
					         . __( 'Save', 'burst' ) . '">
                            <button class="button burst-remove" type="submit"
                            name="burst_remove_multiple['
					         . esc_html( $fieldname ) . ']"
					...
					if ( $title == '' ) {
						$title = __( 'New entry', 'burst' );
					}
					burst_panel( $title, $html );
					?>
			...
			?>
			<button class="button" type="submit" class="burst-add-new-processor"
			        name="burst_add_multiple"
			        value="<?php echo esc_html( $fieldname ) ?>"><?php printf( __( "Add new %s",
					'burst' ), $args['label'] ) ?></button>
			<?php do_action( 'burst_after_field', $args ); ?>
			<?php
		...
		function thirdparties(
			...
			if ( $values ) {
				foreach ( $values as $key => $value ) {
                    ...
                    <input class="button" type="submit" name="burst-save" value="'
					        . __( 'Save', 'burst' ) . '">
                    <button class="button burst-remove" type="submit"
                            name="burst_remove_multiple['
					        . esc_html( $args['fieldname'] ) . ']"
					...
					$title = esc_html( $value['name'] );
					if ( $title == '' ) {
						$title = sprintf( __( 'New entry', 'burst' ) );
					}
					burst_panel( $title, $html );
				}
			}
			...
			<button class="button" type="submit"
			        class="burst-add-new-thirdparty" name="burst_add_multiple"
		...
		function get_value(
			...
			if ( strpos( $source, 'BURST' ) !== false
				...
				$banner = new BURST_COOKIEBANNER( $id );
