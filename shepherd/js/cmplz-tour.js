jQuery(document).ready(function($) {
	...
	var steps = burst_tour.steps;

	plugins_overview_tour.addStep('burst-step-0', {
		classes: 'shepherd-theme-arrows burst-plugins-overview-tour-container shepherd-has-cancel-link',
		attachTo: steps[0]['attach']+' right',
		title: steps[0]['title'],
		text: burst_tour.html.replace('{content}', steps[0]['text']),
		buttons: [
			{
				text: burst_tour.configure,
				classes: 'button button-primary',
				action: function() {
					cancel_tour();
					window.location = burst_tour.configure_link;
				}
			},
			{
				text: burst_tour.startTour,
				classes: 'button button-primary',
				action: function() {
	...
	if (steps.hasOwnProperty(1)) {
		...
		license_tour.addStep('burst-step-1', {
			classes: 'shepherd-theme-arrows burst-plugins-overview-tour-container shepherd-has-cancel-link',
			attachTo: steps[1]['attach'],
			title: steps[1]['title'],
			text: burst_tour.html.replace('{content}', steps[1]['text']),
			buttons: [
				{
					text: burst_tour.nextBtnText,
					classes: 'button button-primary',
					action: function () {
	...
	dashboard_tour.addStep('burst-step-2', {
		classes: 'shepherd-theme-arrows shepherd-has-cancel-link',
		attachTo: steps[2]['attach']+' right',
		title: steps[2]['title'],
		text: burst_tour.html.replace('{content}', steps[2]['text']),
		buttons: [
			{
				text: burst_tour.nextBtnText,
				classes: 'button button-primary',
	...
	wizard_tour.addStep('burst-step-3', {
		classes: 'shepherd-theme-arrows shepherd-has-cancel-link',
		attachTo: steps[3]['attach']+' right',
		title: steps[3]['title'],
		text: burst_tour.html.replace('{content}', steps[3]['text']),
		buttons: [
			{
				text: burst_tour.backBtnText,
				classes: 'button button-primary',
				action: function() {
					window.location = steps[1]['link'];
				}
	...
	cookiebanner_tour.addStep('burst-step-4', {
		classes: 'shepherd-theme-arrows shepherd-has-cancel-link',
		title: steps[4]['title'],
		text: burst_tour.html.replace('{content}', steps[4]['text']),
	...
	integrations_tour.addStep('burst-step-5', {
		classes: 'shepherd-theme-arrows shepherd-has-cancel-link',
		title: steps[5]['title'],
		text: burst_tour.html.replace('{content}', steps[5]['text']),
	...
	settings_tour.addStep('burst-step-6', {
		classes: 'shepherd-theme-arrows shepherd-has-cancel-link',
		title: steps[6]['title'],
		text: burst_tour.html.replace('{content}', steps[6]['text']),
	...
	snapshot_tour.addStep('burst-step-7', {
		classes: 'shepherd-theme-arrows shepherd-has-cancel-link',
		title: steps[7]['title'],
		text: burst_tour.html.replace('{content}', steps[7]['text']),
	...
	finish_tour.addStep('burst-step-8', {
		classes: 'shepherd-theme-arrows shepherd-has-cancel-link',
		title: steps[8]['title'],
		text: burst_tour.html.replace('{content}', steps[8]['text']),
		attachTo: steps[7]['attach'],
		buttons: [
			{
				text: burst_tour.backBtnText,
	...
	if ($('.burst-settings-link').length) {
		plugins_overview_tour.start();
	}
	if ($('#burst_license_key').length) {
		license_tour.start();
	}

	if ($('.burst-dashboard-title').length) {
		dashboard_tour.start();
	}

	if ($('.burst-step-2').length || $('.burst-step-3').length) {
		wizard_tour.start();
	}

	if ($('#cookie-settings .burst-tablinks').length) {
		cookiebanner_tour.start();
	}

	if ($('.burst-scriptcenter').length) {
		integrations_tour.start();
	}

	if ($('.burst-cookie_expiry').length) {
		settings_tour.start();
	}
	if ($('#burst-cookiestatement-snapshot-filter').length) {
		snapshot_tour.start();
	}
	if ($('.burst-step-1').length) {
		finish_tour.start();
	}
